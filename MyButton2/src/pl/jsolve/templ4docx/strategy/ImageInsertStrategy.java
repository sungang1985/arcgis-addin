...
public class ImageInsertStrategy implements InsertStrategy {
	...
	private void insertPicture(XWPFRun r, ImageVariable imageVariable) {
		try {
			r.addPicture(imageVariable.getImageStream(), imageVariable
					.getImageType().getImageType(), imageVariable.getKey(),
					Units.toEMU(imageVariable.getWidth()), Units
							.toEMU(imageVariable.getHeight()));
